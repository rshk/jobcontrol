...
def test_job_crud(storage):  # todo: test list/iter too!
    func = 'jobcontrol.utils.testing:job_simple_echo'
    args = ('foo', 'bar', 'baz')
    kwargs = {'spam': 100, 'eggs': 200, 'bacon': 500}

    job_id = storage.create_job(
        'job-1', function=func, args=args, kwargs=kwargs)

    assert job_id == 'job-1'

    job_info = storage.get_job(job_id)
    assert job_info['id'] == job_id
    assert job_info['config']['function'] == func
    assert job_info['config']['args'] == args
    assert job_info['config']['kwargs'] == kwargs
    assert job_info['config']['dependencies'] == []
    ...
    storage.update_job(job_id, dependencies=['one', 'two', 'three'])

    job_info = storage.get_job(job_id)
    assert job_info['id'] == job_id
    assert job_info['config']['function'] == 'foo:bar'
    assert job_info['config']['args'] == ('foo', 'bar')
    assert job_info['config']['kwargs'] == {'A': 1}
    assert job_info['config']['dependencies'] == ['one', 'two', 'three']
...
def test_job_list_iter_mget(storage):
    ...
    # Create jobs
    job_ids = [storage.create_job(
        function='foo:bar', args=[x]) for x in xrange(5)]

    # Query..
    assert sorted(storage.list_jobs()) == sorted(job_ids)

    _jobs = list(storage.iter_jobs())
    assert sorted([x['id'] for x in _jobs]) == sorted(job_ids)
    for x in _jobs:
        assert x['config']['function'] == 'foo:bar'

    _jobs2 = storage.mget_jobs(job_ids)
    assert sorted(_jobs2) == sorted(_jobs)
...
def test_job_default_values(storage):
    func = 'jobcontrol.utils.testing:job_simple_echo'
    job_id = storage.create_job(function=func)
    ...
    assert job_info['id'] == job_id
    assert job_info['config']['function'] == func
    assert job_info['config']['args'] == ()
    assert job_info['config']['kwargs'] == {}
    assert job_info['config']['dependencies'] == []
    assert storage.get_job_deps(job_id) == []
...
def test_job_deps(storage):
    job_id1 = storage.create_job(
        function='jobcontrol.utils.testing:job_simple_echo', args=['A'])
    job_id2 = storage.create_job(
        function='jobcontrol.utils.testing:job_simple_echo', args=['B'])
    job_id3 = storage.create_job(
        function='jobcontrol.utils.testing:job_simple_echo', args=['C'])
    ...
    assert sorted(_depids(job_id1)) == sorted([job_id2, job_id4])
    ...
    assert sorted(_rdepids(job_id4)) == sorted([job_id1, job_id3])
...
def test_job_build_crud(storage):
    job_id = storage.create_job(function='foo:bar')
